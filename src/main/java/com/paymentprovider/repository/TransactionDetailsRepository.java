...
public interface TransactionDetailsRepository extends JpaRepository<TransactionDetails, Integer> {
	...
	@Query("select sum((a.amount)*(case when a.currency='EUR' THEN  1 WHEN a.currency='USD' THEN 0.9 WHEN a.currency='GBP' THEN 1.2 END)) from TransactionDetails as a where a.date between ?2 and ?3 and a.clientId=?1") 
